...
export function AdminPage({ user, onLogout }: AdminPageProps) {
  ...
  async function fetchData() {
    setLoading(true);
    try {
      const [tr, ur] = await Promise.all([fetch("/tasks"), fetch("/admin/users")]);
  ...
  return (
    <div className="flex h-screen overflow-hidden" style={{ background: "#0A0B0D" }}>
      <aside
        ...
        style={{ background: "#0D1117", borderRight: "1px solid rgba(134,188,37,0.08)" }}
      >
        <div className="px-5 py-5 border-b" style={{ borderColor: "rgba(134,188,37,0.08)" }}>
          <div className="flex items-center gap-2">
            ...
            <span style={{ fontFamily: "Georgia, serif", fontWeight: 700, fontSize: "16px", color: "#F0F2F0" }}>
              Deloitte
            </span>
          </div>
          <div className="mt-1 flex items-center gap-1.5">
            <Shield size={10} style={{ color: "#86BC25" }} />
            <span className="text-xs" style={{ color: "#86BC25" }}>
              Admin Console
            </span>
          </div>
        </div>
        ...
        <div
          ...
          style={{ background: "rgba(134,188,37,0.04)", border: "1px solid rgba(134,188,37,0.08)" }}
        >
          <div className="flex items-center gap-2">
            <div
            ...
            <div className="min-w-0">
              <div className="text-xs font-semibold truncate" style={{ color: "#F0F2F0" }}>
                {user.name}
              </div>
              <div className="text-xs" style={{ color: "#86BC25" }}>
                Administrator
              </div>
            </div>
          </div>
        ...
        <nav className="flex-1 px-3 py-4 space-y-1">
          {[
            ...
            { id: "tasks" as const, icon: CheckCircle2, label: `Tasks (${stats.total})` },
            { id: "users" as const, icon: Users, label: `Users (${users.length})` },
          ].map((item) => (
            <button
              key={item.id}
        ...
        <div className="px-3 pb-5 space-y-1 border-t pt-4" style={{ borderColor: "rgba(134,188,37,0.08)" }}>
          ...
          <button
            onClick={onLogout}
            className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm hover:text-red-400"
            style={{ color: "#6B7A5E" }}
          >
        ...
        >
          <div>
            ...
            <p className="text-xs" style={{ color: "#4A5568" }}>
              Admin Console · {user.company}
            </p>
          </div>
          <button
            onClick={fetchData}
            className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs hover:bg-[#1A1E24]"
                ...
                <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
                  <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
                    {[
                      ...
                      { label: "Total Users", value: users.length, icon: Users, color: "#86BC25" },
                    ].map((stat) => (
                      <div
                        key={stat.label}
                        className="rounded-xl p-4"
                        style={{ background: "#111318", border: "1px solid rgba(134,188,37,0.1)" }}
                      >
                        ...
                        <div className="text-2xl font-bold" style={{ color: "#F0F2F0" }}>
                          {stat.value}
                        </div>
                      </div>
                    ))}
                  ...
                  <div className="rounded-xl p-5" style={{ background: "#111318", border: "1px solid rgba(134,188,37,0.1)" }}>
                    <h3 className="text-sm font-semibold mb-4" style={{ color: "#F0F2F0" }}>
                      Recent Activity
                    </h3>
                        ...
                        >
                          <div className="flex items-center gap-3">
                            {task.status === "DONE" ? (
                              ...
                              <Clock size={14} style={{ color: "#E8A44A" }} />
                            ) : (
                              <AlertCircle size={14} style={{ color: "#4A5568" }} />
                            )}
                            <div>
                              <div className="text-xs font-medium" style={{ color: "#F0F2F0" }}>
                                {task.title}
                              </div>
                              <div className="text-xs" style={{ color: "#4A5568" }}>
                                by {task.createdBy || "system"}
                              </div>
                            </div>
                          </div>
                          <span
                  ...
                  >
                    <table className="w-full text-left" style={{ borderCollapse: "separate", borderSpacing: 0 }}>
                      <thead>
                        <tr style={{ background: "#0D1117" }}>
                          {["ID", "Title", "Priority", "Status", "Deadline", "Created By"].map((h) => (
                            <th key={h} className="px-4 py-3 text-[11px] font-semibold" style={{ color: "#6B7A5E" }}>
                              {h}
                            </th>
                          ))}
                        </tr>
                      ...
                      <tbody>
                        {tasks.map((task) => {
                          const pc =
                            task.priority === "HIGH" ? "#EF4444" : task.priority === "LOW" ? "#86BC25" : "#E8A44A";
                          const sc =
                            task.status === "DONE" ? "#86BC25" : task.status === "IN_PROGRESS" ? "#E8A44A" : "#4A5568";
                          return (
                            <tr key={task.id} className="border-t" style={{ borderColor: "rgba(134,188,37,0.05)" }}>
                              <td className="px-4 py-3 text-xs" style={{ color: "#6B7A5E" }}>
                                {task.id}
                              </td>
                              <td className="px-4 py-3 text-xs font-medium" style={{ color: "#F0F2F0" }}>
                                {task.title}
                              </td>
                              ...
                              <td className="px-4 py-3 text-xs" style={{ color: sc }}>
                                {task.status}
                              </td>
                    ...
                    <table className="w-full text-left">
                      ...
                      <tbody>
                        {users.length > 0 ? (
                            ...
                            <tr key={u.id} className="border-t" style={{ borderColor: "rgba(134,188,37,0.05)" }}>
                              ...
                              <td className="px-4 py-3">
                                <div className="flex items-center gap-2">
                                  ...
                                  <span className="text-xs font-medium" style={{ color: "#F0F2F0" }}>
                                    {u.name}
                                  </span>
                                </div>
                              </td>
                          ...
                          <tr>
                            <td colSpan={5} className="px-4 py-8 text-center text-xs" style={{ color: "#4A5568" }}>
                              No users found — ensure user-service + gateway/admin route is working.
                            </td>
                          </tr>
                        )}
                      </tbody>
